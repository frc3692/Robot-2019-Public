...
public class ArcadeMapping implements DriveMap {
	public DriveData drive() {
		DS4 drive = Robot.getDriveJoy();

		double speedmult;
		if (drive.getRawButton(DSButton.lt)) // Right Stick Speed Boost
			speedmult = .5;
		else if (drive.getRawButton(DSButton.rt)) // Left Stick Slow Speed
			speedmult = 1;
		else // Otherwise Normal Speed
			speedmult = .7;

		double y = -drive.getRawAxis(DSAxis.ly);
